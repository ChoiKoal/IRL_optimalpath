...
# Pytorch ver.
def value_iteration(P_a, rewards, gamma, mapSize, error=0.01, deterministic=True, npy=True):
    """
    ...
    while True:
        ...
        v_s = torch.zeros(N_STATES, N_ACTIONS)
        v_s_s1 = torch.zeros(N_STATES, N_ACTIONS, N_STATES)
        for s in range(N_STATES):
            for a in range(N_ACTIONS):
                for s1 in range(N_STATES):
                    v_s_s1[s][a][s1] = P_a[s, s1, a] * (rewards[s] + gamma * values_tmp[s1])
                v_s[s][a] = torch.sum(v_s_s1[s][a])
            values, _ = torch.max(v_s, 1)

        v_error = torch.zeros(N_STATES)
        for s in range(N_STATES):
            v_error[s] = abs(values[s] - values_tmp[s])
        v_error, _ = torch.max(v_error, 0)
    ...
    if deterministic:
        ...
        for s in range(N_STATES):
            ...
            # get policy
            _, policy = torch.max(v_s, 1)
        return values, policy

    # not changed if deterministic = False
    else:
        ...
        for s in range(N_STATES):
            for a in range(N_ACTIONS):
                for s1 in range(N_STATES):
                    v_s_s1[s][a][s1] = P_a[s, s1, a] * (rewards[s] + gamma * values[s1])
                v_s[s][a] = torch.sum(v_s_s1[s][a])
                # v_s = [sum([P_a[s, s1, a]*(rewards[s] + gamma*values[s1]) for s1 in range(N_STATES)]) for a in range(N_ACTIONS)]

        policy = torch.transpose(v_s / torch.sum(v_s, 0), 0, 1)
        return values, policy
