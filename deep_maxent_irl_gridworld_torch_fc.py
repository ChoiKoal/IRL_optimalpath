...
from deep_maxent_irl_torch_fc import *
...
def main():
  ...
  values, _ = value_iteration.value_iteration(P_a, rewards, GAMMA, error=0.01, deterministic=True, npy = True)
  rewards_gt = rewards_gt.data.numpy()
  #values = values/torch.mean(values)
  #values_gt = values_gt / torch.mean(values_gt)
  values = values.numpy()
